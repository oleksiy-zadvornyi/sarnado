...
export default class Step4 extends React.Component {
  ...
  onChangeFiatAmount = (fiat_amount) => {
    const {rate} = this.props.prevProps;
    const amount = parseFloat(fiat_amount) / parseFloat(rate.price);
    this.setState({
      amount: !isNaN(amount) ? amount.toFixed(2) : '',
      fiat_amount: fiat_amount.replace(',', '.'),
    });
  };

  onChangeAmount = (amount) => {
    const {rate} = this.props.prevProps;
    const fiat_amount = parseFloat(amount) * parseFloat(rate.price);
    this.setState({
      fiat_amount: !isNaN(fiat_amount) ? fiat_amount.toFixed(2) : '',
  ...
  render() {
    const {amount, fiat_amount, errors} = this.state;
    ...
    return (
      <Wrap titleView={<WrapExit title="Вы выставляете ордер на покупку" />}>
        ...
        <Text style={base.text2}>Количество ({fiat.code})</Text>
        <InputIcon
          value={fiat_amount}
          keyboardType="numeric"
          hasError={errors.filter((e) => e === 'fiat_amount').length > 0}
          onChangeText={this.onChangeFiatAmount}
        />
        <Text style={[base.text2, base.margin1]}>
